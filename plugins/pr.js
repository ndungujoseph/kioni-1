...
const ds = "PM Permit opens permission."
...
Asena.addCommand({pattern: 'approve$', desc: ds, onlyPm: true}, (async (message, match) => { 
    ...
    await message.client.sendMessage(message.jid,'Permitted to PM!', MessageType.text);
}));
Asena.addCommand({pattern: 'disapprove$', desc: dds, onlyPm: true}, (async (message, match) => { 
    pmpermit.nopermitacton(message.jid.split("@")[0])
    await message.client.sendMessage(message.jid,'PM Permission Removed!', MessageType.text);
}));
